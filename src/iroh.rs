...
use iroh::endpoint::{
    BindError, Connection, Endpoint, InvalidSocketAddr, QuicTransportConfig, SendDatagramError,
    presets,
};
...
impl Iroh {
    pub async fn new(
        ...
        let mut state_table: HashMap<PublicKey, ConnectionState> =
        ...
        let endpoint = builder.bind().await.map_err(InitError::BuildError)?;
                ...
                match self.state_table.get(&conn.remote_id()) {
                    Some(state) => match state {
                        ...
                        ConnectionState::Connected {
                            task: expired_task,
                            connection: expired_conn,
                            ..
                        } => {
                            info!(
                                "data race: receive outgoing connecting while current state is Connected"
                            );
                            expired_task.abort();
                            expired_conn
                                .close(1u32.into(), b"connection is expired: data race resolver");
                ...
                match self.state_table.get(&conn.remote_id()) {
                    Some(state) => match state {
                        ...
                        ConnectionState::Connected {
                            ...
                            expired_conn.close(
                                1u32.into(),
                                b"connection is expired: peer open another connection",
                            );
                        ...
                        ConnectionState::Connecting(_) => {
                            ...
                            conn.close(1u32.into(), b"peer violated public keys invariant");
                            warn!(
                                "weird connection event: receive incoming connection while current state is Connecting"
                            );
                        }
                ...
                match self.state_table.get(&pub_key) {
                    Some(state) => match state {
                        ConnectionState::Connected { .. } => {
                            ...
                            let _ = self
                                .tx_to_coord
                                .send(IrohEvent::DisconnectedFromPeer(pub_key.clone()))
                                .await;
                        ...
                        ConnectionState::Connecting(_) => {
                            ...
                            debug!(
                                "data race: receive ConnectionEvent::Disconnected while current state is Connecting"
                            );
                        }
                        ...
                        ConnectionState::WaitingIncomingConnection => {
                            // same as Connecting
                            debug!(
                                "data race: receive ConnectionEvent::Disconnected 
                        ...
                        ConnectionState::Disconnected => {
                            ...
                            debug!(
                                "data race: receive ConnectionEvent::Disconnected while current state is Disconnected"
                            )
                        }
                    },
    ...
    async fn handle_core_event(&mut self, event: CoreAction) {
        match event {
            ...
            CoreAction::ConnectToPeer(pub_key) => match self.state_table.get(&pub_key) {
                Some(state) => match state {
                    ConnectionState::Disconnected => {
                        self.init_new_connection(pub_key);
                    }
                    ...
                    ConnectionState::Connected { .. } => {
                        warn!(
                            "weird action from core: ConnectToPeer while current state is Connected"
                        );
                    }

                    ConnectionState::Connecting(_) => {
                        warn!(
                            "weird action from core: ConnectToPeer while current state is Connecting"
                        );
                    }

                    ConnectionState::WaitingIncomingConnection => {
                        warn!(
                            "weird action from core: ConnectToPeer while current state is WaitingIncomingConnection"
                        );
                    }
                },
            ...
            CoreAction::DisconnectFromPeer(pub_key) => {
                match self.state_table.get(&pub_key) {
                    Some(state) => match state {
                        ConnectionState::Connected {
                            ...
                            let _ = self
                                .tx_to_coord
                                .send(IrohEvent::DisconnectedFromPeer(pub_key))
                                .await;
                        }
                ...
                self.state_table
                    .insert(pub_key.clone(), ConnectionState::Disconnected);
            }
        }
    }
    ...
    fn init_new_connection(&mut self, pub_key: PublicKey) {
        if self.endpoint.id() < pub_key {
            self.state_table
                .insert(pub_key, ConnectionState::WaitingIncomingConnection);
            return;
        }

        let task = tokio::spawn(Self::connect_to_peer(
            self.endpoint.clone(),
            pub_key.clone(),
        ...
        self.state_table
            .insert(pub_key, ConnectionState::Connecting(task));
    }
    ...
    async fn init_ready_connection(&mut self, conn: Connection) {
        let _ = self
            .tx_to_coord
            .send(IrohEvent::ConnectedToPeer(conn.remote_id()))
            .await;
        let (tx_packet_channel, rx_packet_channel) = mpsc::channel(CHANNEL_CAPACITY);
        let task = tokio::spawn(Self::on_peer_connected(
    ...
    fn init(&mut self) {
        for pub_key in &self.peers {
            if *pub_key > self.endpoint.id() {
                self.state_table
                    .insert(pub_key.clone(), ConnectionState::WaitingIncomingConnection);
                continue;
            }
            ...
            self.state_table
                .insert(pub_key.clone(), ConnectionState::Connecting(task));
        }
    ...
    async fn on_peer_connected(
        tx_packet_channel: mpsc::Sender<IrohEvent>,
    ...
    async fn connect_to_peer(
        ...
        loop {
            match endpoint.connect(to, ALPN).await {
                Ok(conn) => {
                    let _ = tx_to_manager
                        .send(ConnectionEvent::OutgoingConnection(conn))
                        .await;
                    return;
                }
                Err(e) => {
    ...
    #[instrument(skip_all)]
    async fn accept_connections(endpoint: Endpoint, tx_to_manager: mpsc::Sender<ConnectionEvent>) {
            ...
            match incoming.accept() {
                Ok(ac) => {
                    ...
                    tokio::spawn(async move {
                        match ac.await {
                            Ok(conn) => {
                                let _ = tx_to_manager_clone
                                    .send(ConnectionEvent::IncomingConnection(conn))
                                    .await;
                            }
...
mod tests {
    ...
    async fn test_n_nodes_resilient_mesh() {
        ...
        assert!(
            max_delay >= min_delay,
            "MAX_DELAY must be greater or equal than MIN_DELAY"
        );
            ...
            while no_data_pairs.len() > 0 {
                for (peer_a, peer_b) in &no_data_pairs {
                    if let Some(tx_coord) = controllers.get(peer_a) {
                        let _ = tx_coord
                            .send(CoreAction::SendPacketTo(packet.clone(), peer_b.clone()))
                            .await;
                    }
                }
                ...
                loop {
                    match timeout(Duration::from_millis(500), rx_global_events.recv()).await {
                        Ok(Some((node_id, event))) => match event {
