use iroh::{self, PublicKey, SecretKey};
...
impl Config {
    pub fn from_file(path: impl AsRef<Path>) -> Result<Self, ConfigError> {
        ...
        let log_level = match config_toml.log_level {
            Some(lvl) => match &lvl[..] {
                "trace" | "debug" | "info" | "warn" | "error" | "off" => lvl,
    ...
    fn get_secret_key(path: impl AsRef<Path>) -> Result<SecretKey, SecretKeyError> {
        let data = match fs::read(&path) {
            Ok(data) => data,
            Err(e) if e.kind() == io::ErrorKind::NotFound => match Self::gen_secret_key(path) {
                Ok(sec_key) => {
                    return Ok(sec_key);
                }
                Err(e) => {
...
mod tests {
    ...
    fn test_valid_full_config() {
        ...
        assert_eq!(
            config.peers.get(&peer_ip).unwrap().to_string(),
            peer_pub_key
        );
    }
