...
use crate::vpn_core::{Action, Event, VpnCore};
use bytes::Bytes;
    ...
    async fn handle_actions(&self, actions: Vec<Action>) -> Result<(), ChannelWasClosed> {
        for action in actions {
            match action {
                ...
                Action::DisconnectFromPeer(pub_key) => {
                    // weird, unlikely
                    if self
                        .tx_to_iroh
                        .send(CoreAction::DisconnectFromPeer(pub_key))
                        .await
                        .is_err()
                    {
                        warn!("Channel to iroh was closed");
                ...
                Action::SendPacketTo(packet, to) => {
                    if let Err(e) = self
                        .tx_to_iroh
                        .try_send(CoreAction::SendPacketTo(packet, to))
                    {
                        match e {
