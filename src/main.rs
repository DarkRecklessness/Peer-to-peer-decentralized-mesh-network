use iroh::PublicKey;
use mesh_network::{config::Config, coord::Coordinator, iroh::Iroh, tun::Tun, vpn_core::VpnCore};
...
async fn main() {
    ...
    // Print public key
    info!(
        public_key = cfg.secret_key.public().to_string(),
        "Your public key"
    );
    ...
    let core = VpnCore::new(cfg.node_ipv4.clone(), cfg.peers, final_mtu.into());
    info!("Core was created");

    let (tx_from_coord_to_iroh, rx_from_coord_to_iroh) = mpsc::channel(INTERMOD_CHANNEL_CAPACITY);
    let (tx_from_iroh_to_coord, rx_from_iroh_to_coord) = mpsc::channel(INTERMOD_CHANNEL_CAPACITY);
    let (tx_from_coord_to_tun, rx_from_coord_to_tun) = mpsc::channel(INTERMOD_CHANNEL_CAPACITY);
    let (tx_from_tun_to_coord, rx_from_tun_to_coord) = mpsc::channel(INTERMOD_CHANNEL_CAPACITY);
...
pub fn init_logger(
    ...
    let env_filter =
        EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new(filter_settings));
