...
impl fmt::Display for Action {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Action::SendPacketTo(packet, pub_key) => {
                write!(f, "Send {} bytes to {}", packet.len(), pub_key)
            }
...
impl fmt::Display for SendPacketError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            SendPacketError::IncorrectSrcIp => {
                write!(f, "source IP does not match the assigned tun IP")
            }
            SendPacketError::UnknownDestIp => {
                write!(f, "destination IP is not found in the routing table")
            }
...
impl fmt::Display for RecvPacketError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            RecvPacketError::UnknownSrcIp => {
                write!(f, "source IP is not found in the routing table")
            }
            RecvPacketError::IncorrectDestIp => {
                write!(f, "destination IP does not match local tun IP")
            }
            RecvPacketError::MtuError => write!(f, "received packet length exceeds the MTU limit"),
            RecvPacketError::Spoofing => write!(
                f,
                "anti-spoofing triggered: Source IP does not match the sender's public key"
            ),
...
impl VpnCore {
    ...
    fn send_packet(&mut self, packet: Bytes) -> Vec<Action> {
        ...
        let pub_key = match self.route_table.get(&dest_ip) {
            Some(pk) => pk,
            None => {
                return vec![Action::NoAction(LogEvent::SendPacketError(
                    SendPacketError::UnknownDestIp,
                ))];
            }
        };
